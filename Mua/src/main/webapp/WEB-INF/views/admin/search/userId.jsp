...
<script>
	...
	function getResult(){
		str = $('#searchBar').val();
		$.ajax({
			url: "${root}admin/search/ajax/user",
			type: 'GET',
			dataType: 'json',
			data:{
				str: str,
	...
	function inputResult(result){
		...
		for(var i = 0; i < userList.length; i++){
			searchResult += '<tr class="line">'
						...
						+	'<a href="${root}admin/search/userInfo?user_num=' + userList[i].user_num + '">' + userList[i].user_id
						+	'</a></td>'
						+	'<td class="top_bottom">'
						+	userList[i].user_name
						+	'</td>'
						+	'<td class="top_bottom ellipsis">'
						+	userList[i].user_registdate
						+	'</td></tr>';
		}
